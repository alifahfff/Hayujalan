...
const ModalAkses = ({ visible, onClose, data, roles }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!datas.idRoles) {
      newInputErrors.idRoles = "ID roles harus dipilih";
    }
    if (!datas.namaUser) {
      newInputErrors.namaUser = "Nama User harus diisi";
    }
    if (!datas.Email) {
      newInputErrors.Email = "Email harus diisi";
    }
    if (!datas.Password) {
      newInputErrors.Password = "Password harus diisi";
    }
          ...
          <div className="">
            ...
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div className="flex flex-row justify-between">
                ...
                />
                {inputErrors.namaUser && (
                  <p className="text-red-500 mt-1">{inputErrors.namaUser}</p>
                )}
              </div>
              <div className="flex flex-row justify-between">
                ...
                />
                {inputErrors.Email && (
                  <p className="text-red-500 mt-1">{inputErrors.Email}</p>
                )}
              </div>
              <div className="flex flex-row justify-between">
